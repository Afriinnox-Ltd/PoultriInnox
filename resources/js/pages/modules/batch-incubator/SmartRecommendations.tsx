...
const SmartRecommendations: React.FC<SmartRecommendationsProps> = ({ batch, recommendations }) => {
    ...
    const acceptRecommendation = async (type: string, protocolId: number, notes?: string) => {
        ...
        try {
            ...
            toast.error('Error accepting recommendation. Please try again.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const dismissRecommendation = async (type: string, recommendationId: string, reason: string) => {
        setLoading(true);
        try {
            await router.post(`/batch-incubator/smart-scheduling/batches/${batch.id}/dismiss-recommendation`, {
                ...
                <Card key={index} className="border-l-4 border-l-indigo-500">
                    <CardHeader>
                        <div className="flex items-center space-x-2">
                            ...
                            <Badge className={
                                insight.assessment === 'Excellent' ? 'bg-green-100 text-green-800' :
                    ...
                    <CardContent>
                        ...
                        {insight.achievement_notes && (
                            <div className="bg-green-50 p-3 rounded">
                                <p className="text-sm text-green-800">{insight.achievement_notes}</p>
                            </div>
                        )}
                    </CardContent>
                </Card>
