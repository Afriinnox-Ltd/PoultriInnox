...
export default function SearchPage({ products, categories, vendors, filters, pagination }: SearchPageProps) {
    ...
    const handleSearch = useCallback(() => {
        ...
        router.get('/store/search', Object.fromEntries(params), {
            preserveState: true,
            replace: true,
            preserveScroll: true,
        });
    }, [searchTerm, selectedCategory, selectedVendor, priceMin, priceMax, inStockOnly, selectedCondition, minRating, freeShipping, sortBy]);

    // Track if it's the first render to avoid double-firing on mount
    const isFirstRender = useRef(true);

    // Automatic application for select/checkboxes
    useEffect(() => {
        ...
        handleSearch();
    }, [selectedCategory, selectedVendor, inStockOnly, selectedCondition, minRating, freeShipping, sortBy]);

    // Debounced application for text inputs
    useEffect(() => {
    ...
    const handleSort = (value: string) => {
        setSortBy(value);
        // useEffect will trigger handleSearch
    };
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pt-24">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                    ...
                    <div className="flex items-center gap-4">
                        <select
                            className="px-4 h-10 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
                            value={sortBy}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div className={`
                        lg:col-span-1 
                        ${showMobileFilters ? 'fixed inset-0 z-50 bg-white p-4 overflow-y-auto' : 'hidden lg:block'}
                    `}>
                        {showMobileFilters && (
                        ...
                        <Card className={`border border-gray-200 shadow-sm bg-white ${showMobileFilters ? 'border-0 shadow-none' : 'sticky top-24'}`}>
                            <CardHeader className={`border-b border-gray-200 ${showMobileFilters ? 'px-0' : ''}`}>
                                <CardTitle className="flex items-center text-lg">
                                    <Filter className="h-5 w-5 mr-2 text-emerald-600" />
                                    Filters
                                </CardTitle>
                            </CardHeader>
                            <CardContent className={`space-y-6 pt-6 ${showMobileFilters ? 'px-0' : ''}`}>
                                {/* Search */}
                                <div>
                                    ...
                                    <Input
                                        ...
                                        className="border-gray-300 focus:border-emerald-500 focus:ring-emerald-500 h-10"
                                    />
                                </div>
                                ...
                                <div>
                                    ...
                                    <select
                                        className="w-full px-3 h-10 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-emerald-500"
