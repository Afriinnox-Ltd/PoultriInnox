...
class OrderAdminController extends Controller
{
    ...
    public function updateStatus(Request $request, Order $order)
    {
        ...
        switch ($validated['status']) {
            case 'confirmed':
                ...
                $this->confirmOrder($request, $order); // Ensure order is confirmed
                break;
            case 'processing':
    ...
    /**
     * Confirm an order for vendor management.
     */
    public function confirmOrder(Request $request, Order $order)
    {
        ...
        if (in_array($order->status, ['cancelled', 'refunded'])) {
            return back()->with('error', 'Cannot confirm a cancelled or refunded order.');
        }

        $order->markAsAdminConfirmed(Auth::id());
        ...
        return back()->with('success', 'Order confirmed successfully. Vendor can now manage this order.');
    }
