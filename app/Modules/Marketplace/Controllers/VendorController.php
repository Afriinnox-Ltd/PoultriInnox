...
class VendorController extends Controller
{
    ...
    public function publicRegister(Request $request)
    {
        ...
        try {
            ...
            // 4. Send vendor application confirmation email
            try {
                Mail::to($vendor->business_email)->send(new VendorApplicationReceived($vendor));
            } catch (\Exception $e) {
                \Log::error('Failed to send vendor application email', ['vendor_id' => $vendor->id, 'error' => $e->getMessage()]);
            }

            // 5. Assign Free Subscription
            ...
            try {
                $adminEmail = config('mail.admin_email', 'admin@agriinnox.com');
                Mail::to($adminEmail)->send(new NewVendorRegistration($vendor));
            } catch (\Exception $e) {
                \Log::error('Failed to send admin vendor notification', ['vendor_id' => $vendor->id, 'error' => $e->getMessage()]);
            }
        ...
        if ($existingVendor) {
            ...
            switch ($existingVendor->status) {
                ...
                case 'changes_requested':
                    // Allow editing for changes requested
                    break;
                default:
                    return redirect()->route('marketplace.vendor.dashboard')
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'business_documents.*' => 'file|mimes:pdf,jpg,jpeg,png,doc,docx|max:5120',
        ...
        if (empty($request->business_documents) && empty($request->existing_documents)) {
            return back()->withErrors(['business_documents' => 'At least one verification document is required.'])->withInput();
        }

        // Create vendor profile
        $vendorData = collect($validated)->except(['name', 'email', 'password', 'password_confirmation', 'phone', 'business_documents', 'existing_documents'])->toArray();
        ...
        if ($request->has('existing_documents')) {
            $documentPaths = $request->existing_documents;
        }
        ...
        $originalSlug = $vendorData['slug'];
        $counter = 1;
        while (Vendor::where('slug', $vendorData['slug'])->where('id', '!=', $existingVendor->id ?? 0)->exists()) {
            $vendorData['slug'] = $originalSlug.'-'.$counter;
            $counter++;
        }
        ...
        if ($freePlan && ! $vendor->subscription()->exists()) {
            Subscription::create([
                'vendor_id' => $vendor->id,
                'plan_id' => $freePlan->id,
                'start_date' => now(),
    ...
    public function approve(Vendor $vendor)
    {
        ...
        try {
            ...
            \Log::error('Failed to send vendor approval email', ['vendor_id' => $vendor->id, 'error' => $e->getMessage()]);
        }
    ...
    public function reject(Vendor $vendor, Request $request)
    {
        ...
        // Notify vendor of rejection via email
        try {
            Mail::to($vendor->business_email)->send(new VendorRejected($vendor, $validated['rejection_reason']));
        } catch (\Exception $e) {
            \Log::error('Failed to send vendor rejection email', ['vendor_id' => $vendor->id, 'error' => $e->getMessage()]);
        }
